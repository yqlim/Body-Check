    ...
    _createClass(Validation, [{
      ...
      value: function run(values) {
        ...
        return new Promise_constructor(function (resolve, reject) {
          ...
          start.call(this, values).then(resolve).catch(reject);

          function start(values) {
            var _this2 = this;

            return new Promise_constructor(function (res, rej) {
              _newArrowCheck(this, _this2);

              runner.call(this, values).catch(rej);

              function runner(valueSet) {
                var _this3 = this;
                ...
                if (!caseConfig) {
                  rej(new ReferenceError("Value of \"".concat(name, "\" cannot be validated because its case is not found.")));
                  return;
                }
                ...
                var validator = caseConfig.validator,
                    ...
                    context = caseConfig.context; // Force array

                var params = caseConfig.params === void 0 ? [] : Array.isArray(caseConfig.params) ? caseConfig.params : [caseConfig.params];
                return Promise_constructor.resolve(validator.call.apply(validator, [context, value].concat(_toConsumableArray(params)))).catch(rej).then(function (result) {
